...
- Reusable: distributed as an internal module under `src/framework`, importable by any service.
...
## Acceptance Criteria
- Framework code is isolated under `src/framework/*` and exposes a single client abstraction.
...
- `src/framework/*.ts` (client, retry, circuit breaker wrapper, logger, tracing, idempotency, config, errors)
...
- Assumption: OpenTelemetry SDK is acceptable; if a full collector is out of scope, `traceparent` propagation + structured logs are sufficient for the assessment.
...
- Open: Logging sink (stdout JSON only vs Application Insights/OTel exporter) for the demo.
